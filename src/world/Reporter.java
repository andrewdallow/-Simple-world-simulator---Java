...
/**
 * Class which produces a textual report of a model world. Not automatically
 * updated when model state changes. Polls model to obtain current data and
 * state.
 * @see World
...
public class Reporter implements Reportable
{
	...
	/**
	 * Method to report places by printing them to a destination
	 * (PrintWriter, e.g., file, console).
	 *
	 */
	private void reportPlaces() 
	{
		destination.println("Places:");
		Collection<Place> places = world.places();
	
		for (Place r : places) 
		{
			destination.println(r);

			// Since rooms may contain things we also need to report things for
			// each room (if room is not empty).
	...
	/**
	 * Method to report items (or things) by printing them to a destination 
	 * (PrintWriter, e.g., file, console).	
	 * 
	 */
	...
	/**
	 * Method to report actors (persons) by printing them to a destination 
	 * (PrintWriter, e.g., file, console).	
	 * 
	 */	
	private void reportActors() 
	{
		...
		for (Actor p : actors) 
		{
			...
			// Since persons may be carrying items we also need to report items
			// for each person (if person is carrying any items).
	...
	/**
	 * Constructor
	 * @param fileOutput Determines whether output is printed to file or to 
	 * standard output stream (i.e., console).
	 * 	
	 */	
			...
			switch (fileChooser.showSaveDialog(null)) 
			{
				...
				case JFileChooser.APPROVE_OPTION:
					...
					} catch (IOException e) 
					{
						e.printStackTrace();
						System.err.println(
								"Error occurred in output file selection");
						System.exit(-1);
					}
					break;
				default:
